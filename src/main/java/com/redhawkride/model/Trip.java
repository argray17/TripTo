...
public class Trip {
  ...
  private Double estimatedTripCost, finalTripCost;
  ...
  double getDistance(){
    ...
    // apply formula
    double a = Math.pow(Math.sin(dLatitude / 2), 2) + Math.pow(Math.sin(dLongitude / 2), 2) * Math.cos(latitudeBegin) * Math.cos(latitudeEnd);
    double rad = 6371;
  ...
  double estimateTripCost(Location beginLocation, Location endLocation){
    estimatedTripCost = 1.25 * getDistance();
    return estimatedTripCost;
  }

  double calcTripCost(Date startTime, Date endTime){
    long time = ((endTime.getTime() - startTime.getTime()) / 1000) % 60; //time difference converted from milliseconds to sec
    finalTripCost = 0.25 * time + 0.5 * getDistance();
    return finalTripCost;
  }
}
